...
#     coeffs_tensor = torch.cat((id_coeffs_tensor, exp_coeffs_tensor, tex_coeffs_tensor,
#                                angles_tensor, gammas_tensor, translations_tensor), dim=0)
...
#     output_vis = torch.tensor(output_vis_numpy_raw / 255., dtype=torch.float32).permute(0, 3, 1, 2).to(device)
...
#     output_vis_black_bg = torch.tensor(output_vis_numpy_raw_black_bg / 255., dtype=torch.float32).permute(0, 3, 1, 2).to(device)
...
def render_image(save_folder, name, k, vectors, new_vector0, image_path, lm_path, target="closest"):
        ...
        # generate render--original pose for input image    
        id_coeffs = new_vector0[:80]
...
def process_file(mat_file):
    ...
    image_folder = "your_location/image"
    detections_folder = "your_location/image/detections"
    save_folder = "your_location/coef2mesh_demo"
    ...
    for k in [0, 10, 30, 50, 70, 90]:
        ...
        # print(f"Processing {name} with k={k}")
        try:
            render_image(save_folder, name, k, vectors, new_vector0_closest, image_path, lm_path, target='cloest')
        except Exception as e:
            print(f"Error in render_image (closest) for {name} k={k}: {e}")
        try:
            render_image(save_folder, name, k, vectors, new_vector0_race, image_path, lm_path, target='race')
        except Exception as e:
            print(f"Error in render_image (race) for {name} k={k}: {e}")
        try:
            render_image(save_folder, name, k, vectors, new_vector0_aisa, image_path, lm_path, target='asia')
        except Exception as e:
            print(f"Error in render_image (asia) for {name} k={k}: {e}")
        try:
            render_image(save_folder, name, k, vectors, new_vector0_euro, image_path, lm_path, target='euro')
        except Exception as e:
            print(f"Error in render_image (euro) for {name} k={k}: {e}")

def main():
    mat_input_path_local = mat_input_path
    mat_files = [f for f in os.listdir(mat_input_path_local) if f.endswith('.mat')]
