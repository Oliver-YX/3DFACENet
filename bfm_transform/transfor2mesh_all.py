...
# Set to your coefficient Excel path (e.g. bfm_coefficient/all_coef.xlsx)
all_file_path = os.path.join(os.path.dirname(__file__), "..", "bfm_coefficient", "all_coef.xlsx")
# Or use absolute placeholder: all_file_path = "your_location/all_coef.xlsx"
...
def load_vectors_from_mat(name, directory):
    ...
    # Extract vectors and build dict
    vectors = {
...
def generate_coeff(id_coeffs, exp_coeffs, tex_coeffs, angles, gammas, translations):
    ...
    id_coeffs_tensor = torch.from_numpy(np.array(id_coeffs))
    exp_coeffs_tensor = torch.from_numpy(np.array(exp_coeffs))
    tex_coeffs_tensor = torch.from_numpy(np.array(tex_coeffs))
    angles_tensor = torch.from_numpy(np.array(angles))
    gammas_tensor = torch.from_numpy(np.array(gammas))
    translations_tensor = torch.from_numpy(np.array(translations))
    # use torch.cat to connect tensors
    coeffs_tensor = torch.cat((id_coeffs_tensor, exp_coeffs_tensor, tex_coeffs_tensor, angles_tensor, gammas_tensor, translations_tensor), dim=0)

    # reshape tensor
    coeffs_tensor = coeffs_tensor.reshape(1, -1)
...
def compute_visuals_rec_only(pred_face,pred_mask,input_img):
        ...
        black_background = np.zeros((batch_size, channels, height, width), dtype=np.float32)        
        ...
        ###create a projected face of a black background
        output_vis_black_bg = pred_face * pred_mask + (1 - pred_mask) * black_background
        output_vis_numpy_raw_black_bg = 255. * output_vis_black_bg.detach().cpu().permute(0, 2, 3, 1).numpy()
        output_vis_black_bg = torch.tensor(
                output_vis_numpy_raw_black_bg / 255., dtype=torch.float32
            ).permute(0, 3, 1, 2).to(device)
        output_vis_black_bg = output_vis_black_bg.squeeze(0)
        input_img_numpy = input_img_numpy.squeeze(0)
        return output_vis,output_vis_black_bg,input_img_numpy
...
def move_towards_closest_vector(vector0, centers, k, name=None):
    """
    Move vector0 towards the closest center in centers by k percent, based on the name parameter.

    :param vector0: The original vector.
    :param centers: A dictionary of center vectors.
    :param k: The percentage of the distance to move towards the closest center.
    :param name: A string indicating the name parameter.
    :return: The updated vector0.
    """
    # select the correct cluster center
    if name and "AF" in name:
        center = centers['AF']
        print("close to AF center")
    elif name and "CF" in name:
        center = centers['CF']
        print("close to CF center")
    elif name and "AM" in name:
        center = centers['AM']
        print("close to AM center")
    elif name and "CM" in name:
        center = centers['CM']
        print("close to CM center")
    else:
         # calculate the distance to each center
        distances = {name: calculate_distance(vector0, center) for name, center in centers.items()}
        print(distances)

        # find the closest center
        closest_center_name = min(distances, key=distances.get)
        print(f"close to {closest_center_name} center")
...
def move_towards_closest_vector_fix(vector0, centers, k, name=None):
    """
    Move vector0 towards the closest center in centers by k percent, based on the name parameter.
...
def process_file(all_file_path, save_file_path, centers):
    """
    Process the Excel file to calculate distances and save to a new file.

    :param all_file_path: Path to the input Excel file.
    :param save_file_path: Path to the output Excel file.
    :param centers: A dictionary of cluster centers.
    """
    
    data = pd.read_excel(all_file_path)
    results = pd.DataFrame(columns=['filename', 'score', 'distance'])

    for index, row in data.iterrows():
        filename = row['filename']
        score = row['score']
        vector0 = row[2:]  #shape & texture
        ...
        distance = calculate_distance(np.array(vector0), center)
        results = results.append({'filename': filename, 'score': score, 'distance': distance}, ignore_index=True)
    
    results.to_excel(save_file_path, index=False)



def analyze_data(file_path, save_stat_path, save_plot_path):
    ...
    scaler = MinMaxScaler()
    data_normalized = scaler.fit_transform(data[['distance', 'score']])
    # data_normalized = pd.DataFrame(data_normalized, columns=['distance', 'score'])
    
    distance = data_normalized[:,0]
    # score = data_normalized[:,1]
    
    correlation_normalized, p_value_normalized = pearsonr(distance, score)
    # correlation_normalized, p_value_normalized = spearmanr(distance, score)

    data_draw = pd.DataFrame({'distance': distance, 'score': score})
    plt.figure(figsize=(10, 6))
    sns.scatterplot(x='distance', y='score', data=data_draw)
    plt.title(f'Distance vs. Score (Pearson Correlation: {correlation_normalized:.2f})', fontsize=18)
    ...
    with open(save_stat_path, 'w') as f:
        f.write(f"Spearman Correlation Coefficient (Normalized): {correlation_normalized}\n")
        f.write(f"P-value (Normalized): {p_value_normalized}\n")
        f.write(f"Mean of Score: {score_mean}\n")
        f.write(f"Standard Deviation of Score: {score_std}\n")
        f.write(f"Mean of Distance: {distance_mean}\n")
        f.write(f"Standard Deviation of Distance: {distance_std}\n")


def analyze_data_5times(file_path, save_stat_path, save_plot_base_path):
    data = pd.read_excel(file_path)
    
    with open(save_stat_path, 'w') as stat_file:
        for seed in range(42,47):
            
            data_sampled = data.sample(frac=0.05, random_state=seed)
            score = data_sampled['score']
            # distance = data_sampled['distance']
    
            scaler = MinMaxScaler()
            data_normalized = scaler.fit_transform(data_sampled[['distance', 'score']])
            distance = data_normalized[:, 0]
            # score = data_normalized[:, 1]
            ...
            plt.figure(figsize=(10, 6))
            sns.scatterplot(x='distance', y='score', data=pd.DataFrame({'distance': distance, 'score': score}))
            plt.title(f'Distance vs. Score (Seed {seed}, Pearson Correlation: {correlation_normalized:.2f})', fontsize=18)
            plt.xlabel('Distance', fontsize=14)
            plt.ylabel('Score', fontsize=14)

            save_plot_path = f"{save_plot_base_path}_seed{seed}.png"
            plt.savefig(save_plot_path)
            plt.close()

            stat_file.write(f"Seed {seed} - Spearman Correlation Coefficient (Normalized): {correlation_normalized}, P-value: {p_value_normalized}\n")
...
# save_path = "/workspace/AdBRC/xieyuan/3Dtask/Project_3Dlandmark/bfm_transform/coef2mesh_demo/clusering/beauty/3.5/2"
# if not os.path.exists(save_path):
#     os.makedirs(save_path)

def process_files_in_folder(folder_path, save_folder, facemodel, mesh_rasterizer, renderer, lm3d_std, centers, k=50):
    ...
    for image_file in image_files:
        ...
        mat_path = os.path.join(mat_folder, f"{name}.mat")
        
        vectors = load_vectors_from_mat( f"{name}.mat", mat_folder)
        
        vector0 = np.concatenate([vectors["id"], vectors["tex"]])
        new_vector0 = move_towards_closest_vector(vector0, centers, k, name)
        ...
        # input_image_numpy = tensor2im(input_img_raw)
        # input_img_path = os.path.join(save_folder, f'{name}_gt.png')
        # save_image(input_image_numpy, input_img_path)
        
        # image_numpy = tensor2im(output_img)
        # img_path = os.path.join(save_folder, f'{name}_{k}.png')
        # save_image(image_numpy, img_path)
        
        image_numpy_black_bg = tensor2im(output_img_black_bg)
        img_path_black_bg = os.path.join(save_folder, f'{name}_{k}_black_bg.png')
        save_image(image_numpy_black_bg, img_path_black_bg)

# save_path = "./beautify_demo/meta_faces"
folder_path = "your_input_image_folder"
# mat_input_path = 'reconstructed_face_folder/mat'
save_folder = "your_output_folder/beautified"
    
if not os.path.exists(save_folder):
    os.makedirs(save_folder)
    
process_files_in_folder(folder_path, save_folder, facemodel, mesh_rasterizer, renderer, lm3d_std, centers, k=90)
# process_files_in_folder(folder_path, save_folder, facemodel, mesh_rasterizer, renderer, lm3d_std, centers, k=0)
