...
raster_settings = RasterizationSettings(
    image_size=int(2 * 112.0), blur_radius=0.0, faces_per_pixel=1, bin_size=None
)
...
def generate_coeff(id_coeffs, exp_coeffs, tex_coeffs, angles, gammas, translations):
    id_tensor = torch.from_numpy(np.array(id_coeffs))
    exp_tensor = torch.from_numpy(np.array(exp_coeffs))
    tex_tensor = torch.from_numpy(np.array(tex_coeffs))
    angles_tensor = torch.from_numpy(np.array(angles))
    gammas_tensor = torch.from_numpy(np.array(gammas))
    trans_tensor = torch.from_numpy(np.array(translations))
    coeffs = torch.cat((id_tensor, exp_tensor, tex_tensor,
                        angles_tensor, gammas_tensor, trans_tensor), dim=0)
...
def compute_visuals_rec_only(pred_face, pred_mask, input_img):
    ...
    black_background = torch.zeros((batch_size, channels, height, width), device=device, dtype=input_img.dtype)
    
    # compute reconstruct face
    ...
    input_img_np = input_img.squeeze(0).cpu().numpy()
    return output_vis, output_vis_black_bg, input_img_np
...
def adaptive_k(score, distance, alpha=5):
    """
    adaptive calculation k: k = exp(-alpha * score / distance^2) * 100
    return value is moving percentage (0~100).
    """
    return np.exp(-alpha * score / (distance**2)) * 100


# —— beauty required functions and constants —— #
...
def adaptive_k(score, distance, alpha=20):
    eps = 1e-6
...
def generate_coeff(id_coeffs, exp_coeffs, tex_coeffs, angles, gammas, translations, device):
    import torch
    id_t  = torch.from_numpy(id_coeffs).to(device)
    exp_t = torch.from_numpy(exp_coeffs).to(device)
    tex_t = torch.from_numpy(tex_coeffs).to(device)
    ang_t = torch.from_numpy(angles).to(device)
    gam_t = torch.from_numpy(gammas).to(device)
    trn_t = torch.from_numpy(translations).to(device)
    coeffs = torch.cat((id_t, exp_t, tex_t, ang_t, gam_t, trn_t), dim=0)
    return coeffs.reshape(1, -1)


# def save_mesh_real(pred_vertex, pred_color, face_buf, file_path):
#     """
#     export .obj
#     """
...
def save_mesh_real_nocolor(pred_vertex, face_buf, file_path):
    shape = pred_vertex.cpu().numpy()[0]
    shape[..., -1] = 10 - shape[..., -1]
    tri = face_buf.cpu().numpy()
    mesh = trimesh.Trimesh(vertices=shape, faces=tri)
    mesh.export(file_path)


def beautify_from_mat(mat_path, save_folder,
                      facemodel, lm3d_std, centers, device,
                      mode='human', k=50, alpha=5, score_file=None):
    """
    Beautify from a single .mat file; save name.obj and name.txt.
    """
    name = os.path.splitext(os.path.basename(mat_path))[0]

    vectors = load_vectors_from_mat(f"{name}.mat", os.path.dirname(mat_path))
    vector0 = np.concatenate([vectors["id"], vectors["tex"]])

    if   "AF" in name: center = centers['AF']
    elif "CF" in name: center = centers['CF']
    elif "AM" in name: center = centers['AM']
    elif "CM" in name: center = centers['CM']
    else:
        dists   = {n: calculate_distance(vector0, c) for n, c in centers.items()}
        closest = min(dists, key=dists.get)
        center  = centers[closest]
    distance = calculate_distance(vector0, center)

    if mode == 'auto':
        if score_file is None:
            raise ValueError("auto mode must provide --score_file")
        df = pd.read_excel(score_file)
        row = df[df['filename'] == f"{name}.mat"]
        score = float(row['score'].values[0]) if not row.empty else 0.0
        current_k = adaptive_k(score, distance, alpha)
        
        obj_folder = os.path.join(save_folder, "auto")
        os.makedirs(obj_folder, exist_ok=True)
        obj_path = os.path.join(obj_folder, f"{name}.obj")
        txt_path = os.path.join(obj_folder, f"{name}.txt")
    else:
        current_k = k
        
        obj_folder = os.path.join(save_folder, str(k))
        os.makedirs(obj_folder, exist_ok=True)
        obj_path = os.path.join(obj_folder, f"{name}.obj")
        txt_path = os.path.join(obj_folder, f"{name}.txt")

    # move vector
    new_vec = move_towards_closest_vector(vector0, centers, current_k, name)
    # new_vec = move_towards_closest_vector_euro(vector0, centers, current_k, name)

    # generate coeff+render
    inp = generate_coeff(
        new_vec[:80],
        vectors["exp"],
        new_vec[80:],
    ...
    pred_v, pred_t, pred_c, _, pred_lm3d = facemodel.compute_for_render(inp)

    
    # save_mesh_real(pred_v, pred_c, facemodel.face_buf.float(), obj_path)
    save_mesh_real_nocolor(pred_v, facemodel.face_buf.float(), obj_path)

    # write 3D landmark TXT
    lm3d = pred_lm3d.cpu().numpy()[0]  # (L,3)
    indices = [36, 39, 42, 45, 33, 48, 54]
    lm3d_selected = lm3d[indices, :] 
    np.savetxt(txt_path, lm3d_selected, fmt="%.6f")

def parse_args():
    parser = argparse.ArgumentParser(
        description="Batch face beautification: save .obj and 3D landmark .txt"
    )
    parser.add_argument("--input_folder", required=True,
                        help="Sample name (no extension); script looks for <name>.mat under ROOT")
    parser.add_argument("--output_folder", required=True,
                        help="Folder to save .obj and .txt")
...
if __name__ == '__main__':
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    args = parse_args()
    root = "your_location/results/examples"
    mat_path = os.path.join(root, args.input_folder + ".mat")
    ...
    # beautify_from_mat(
    #     mat_path, save_folder, facemodel, lm3d_std,
    #     centers, device, mode='auto', k=30, alpha=20 # alpha is 5 by default, can be customized by alpha=
    # )
    
    beautify_from_mat(
        mat_path, save_folder, facemodel, lm3d_std,
        centers, device, mode='human', k=0, alpha=20 # alpha is 5 by default, can be customized by alpha=
    )

    beautify_from_mat(
        mat_path, save_folder, facemodel, lm3d_std,
        centers, device, mode='human', k=10, alpha=20 # alpha is 5 by default, can be customized by alpha=
    )

    beautify_from_mat(
        mat_path, save_folder, facemodel, lm3d_std,
        centers, device, mode='human', k=30, alpha=20 # alpha is 5 by default, can be customized by alpha=
    )

    beautify_from_mat(
        mat_path, save_folder, facemodel, lm3d_std,
        centers, device, mode='human', k=50, alpha=20 # alpha is 5 by default, can be customized by alpha=
    )

    beautify_from_mat(
        mat_path, save_folder, facemodel, lm3d_std,
        centers, device, mode='human', k=70, alpha=20 # alpha is 5 by default, can be customized by alpha=
    )

    beautify_from_mat(
        mat_path, save_folder, facemodel, lm3d_std,
        centers, device, mode='human', k=90, alpha=20 # alpha is 5 by default, can be customized by alpha=
    )