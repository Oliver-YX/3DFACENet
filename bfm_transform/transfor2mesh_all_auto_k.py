...
def save_mesh_real(pred_vertex, pred_color, face_buf, file_path):
    # pred_vertex: [1, N, 3], pred_color: [1, N, 3], face_buf: [M, 3]
    shape = pred_vertex.cpu().numpy()[0]
    shape[..., -1] = 10 - shape[..., -1]
    color = pred_color.cpu().numpy()[0]
    tri = face_buf.cpu().numpy()
...
def move_towards_closest_vector(vector0, centers, k, name=None):
    if name and "AF" in name:
        center = centers['AF']; print("close to AF center")
    elif name and "CF" in name:
        center = centers['CF']; print("close to CF center")
    elif name and "AM" in name:
        center = centers['AM']; print("close to AM center")
    elif name and "CM" in name:
        center = centers['CM']; print("close to CM center")
    else:
        dists = {n: calculate_distance(vector0, c) for n,c in centers.items()}
        closest = min(dists, key=dists.get)
        center = centers[closest]; print(f"close to {closest} center")
...
def process_files_in_folder(folder_path, save_folder, facemodel,
                            mesh_rasterizer, renderer, lm3d_std,
                            centers, mode='human', k=50, alpha=5):
    """
    mode: 'human' use the input k; 'auto' use adaptive_k to calculate k
    """
    # if mode is auto, read the score table
    if mode == 'auto':
    ...
    for image_file in image_files:
        ...
        lm_path = os.path.join(folder_path, "detections", f"{name}.txt")
        mat_dir = os.path.join(folder_path, "mat")
        vectors = load_vectors_from_mat(f"{name}.mat", mat_dir)

        vector0 = np.concatenate([vectors["id"], vectors["tex"]])

        # select the center and calculate the distance
        if "AF" in name:
            center = centers['AF']
        elif "CF" in name:
            center = centers['CF']
        ...
        # select k based on the mode
        if mode == 'auto':
            row = score_df[score_df['filename'] == image_file]
            score_val = float(row['score'].values[0]) if not row.empty else 0.0
            current_k = adaptive_k(score_val, distance, alpha)
            suffix = 'auto'
        else:
            current_k = k
            suffix = k
        ...
        # move the vector
        new_vector0 = move_towards_closest_vector(vector0, centers, current_k, name)

        # generate the render--standard pose
        ...
        # generate the render--original pose
        id_coeffs = new_vector0[:80]
        ...
        input_coeff = generate_coeff(id_coeffs, exp_coeffs, tex_coeffs,
                                     angles, gammas, translations)
        pred_vertex, pred_tex, pred_color, pred_lm, pred_lm3dland = facemodel.compute_for_render(input_coeff)
        pred_mask, _, pred_face = renderer(mesh_rasterizer,
                                           pred_vertex.float(),
                                           facemodel.face_buf.float(),
                                           feat=pred_color.float())
        input_img, _ = read_data(image_path, lm_path, lm3d_std)
        input_img = input_img.to(device)
        _, output_img_black_bg, _ = compute_visuals_rec_only(pred_face, pred_mask, input_img)

        image_numpy_bb = tensor2im(output_img_black_bg)
        img_name = f'{name}_{suffix}_black_bg.png'
        save_image(image_numpy_bb, os.path.join(save_folder, img_name))
        obj_name = f'{name}_{suffix}.obj'
        save_mesh_real(pred_vertex, pred_color, facemodel.face_buf.float(), os.path.join(save_folder, obj_name))



folder_path = "your_location/image"
save_folder = "your_location/beautified_bfm_pose"
# save_folder = os.path.join(folder_path, "beautified")
os.makedirs(save_folder, exist_ok=True)

# if you want to use adaptive k, please set mode='auto'
process_files_in_folder(
    folder_path, save_folder, facemodel,
    mesh_rasterizer, renderer, lm3d_std,
    centers, mode='auto', k=90, alpha=20 # alpha is 5 by default, can be customized by alpha=
)

process_files_in_folder(
    folder_path, save_folder, facemodel,
    mesh_rasterizer, renderer, lm3d_std,
    centers, mode='human', k=0, alpha=20 # alpha is 5 by default, can be customized by alpha=
)

process_files_in_folder(
    folder_path, save_folder, facemodel,
    mesh_rasterizer, renderer, lm3d_std,
    centers, mode='human', k=10, alpha=20 # alpha is 5 by default, can be customized by alpha=
)

process_files_in_folder(
    folder_path, save_folder, facemodel,
    mesh_rasterizer, renderer, lm3d_std,
    centers, mode='human', k=30, alpha=20 # alpha is 5 by default, can be customized by alpha=
)

process_files_in_folder(
    folder_path, save_folder, facemodel,
    mesh_rasterizer, renderer, lm3d_std,
    centers, mode='human', k=50, alpha=20 # alpha is 5 by default, can be customized by alpha=
)

process_files_in_folder(
    folder_path, save_folder, facemodel,
    mesh_rasterizer, renderer, lm3d_std,
    centers, mode='human', k=70, alpha=20 # alpha is 5 by default, can be customized by alpha=
)

process_files_in_folder(
    folder_path, save_folder, facemodel,
    mesh_rasterizer, renderer, lm3d_std,
    centers, mode='human', k=90, alpha=20 # alpha is 5 by default, can be customized by alpha=
)