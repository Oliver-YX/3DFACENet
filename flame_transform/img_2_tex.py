...
if run_this_file == 1:
    ...
    savefolder = "your_location/out_data"
    inputpath = "your_location/in_data"
    iscrop = True
    detector = 'fan'
    sample_step = 10
    useTex = True
...
def main():
    ...
    for i in tqdm(range(len(testdata))):
        ...
        with torch.no_grad():
            ...
            cv2.imwrite(os.path.join(savefolder,  name + '0.png'), correct_tex_rgb) 
            # correct_tex = uv_tex[0].permute(1,2,0).detach().cpu()

            # TODO Perlin Noise, Blending

            correct_tex_rgb = cv2.cvtColor(correct_tex.numpy().astype(np.uint8), cv2.COLOR_BGR2RGB)
            cv2.imwrite(os.path.join(savefolder,  name + '1.png'), correct_tex_rgb) 
            cv2.imwrite(os.path.join(savefolder,  name+'2.png'), cv2.cvtColor(correct_tex.numpy()*255, cv2.COLOR_BGR2RGB))
            # cv2.imwrite(os.path.join(savefolder, name, name+'.png'), cv2.cvtColor(correct_tex.numpy()*255, cv2.COLOR_BGR2RGB))
            # print('Done')
