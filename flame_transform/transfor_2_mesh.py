...
def load_vectors_from_mat(name, directory, device):
    file_path = os.path.join(directory, name)
    mat_data = loadmat(file_path)
    vectors = {
        "cam": torch.from_numpy(mat_data["cam"]).float().to(device),
        "detail": torch.from_numpy(mat_data["detail"]).float().to(device),
        "exp": torch.from_numpy(mat_data["exp"]).float().to(device),
        "images": torch.from_numpy(mat_data["images"]).to(device),
        "light": torch.from_numpy(mat_data["light"]).float().to(device),
        "mica_cam": torch.from_numpy(mat_data["mica_cam"]).float().to(device),
        "mica_exp": torch.from_numpy(mat_data["mica_exp"]).float().to(device),
        "mica_light": torch.from_numpy(mat_data["mica_light"]).float().to(device),
        "mica_pose": torch.from_numpy(mat_data["mica_pose"]).float().to(device),
        "mica_shape": torch.from_numpy(mat_data["mica_shape"]).float().to(device),
        "pose": torch.from_numpy(mat_data["pose"]).float().to(device),
        "shape": torch.from_numpy(mat_data["shape"]).float().to(device),
        "tex": torch.from_numpy(mat_data["tex"]).float().to(device),
    }
...
def move_towards_closest_vector_fix(vector0, centers, k, name, device):
    ...
    # select the correct clustering center
...
def move_towards_closest_vector(vector0, centers, k, name, device):
    ...
    if 'AF' in name:
        ...
        # extract the center vector and convert to torch.Tensor
...
def move_towards_closest_vector_C(vector0, centers, k, name, device):
    # ensure vector0 is torch.Tensor and move to the same device
    if not isinstance(vector0, torch.Tensor):
        vector0 = torch.from_numpy(vector0)
...
def process_file(all_file_path, save_file_path, centers):
    ...
    for index, row in data.iterrows():
        ...
        # select the clustering center based on the filename
        if "AF" in filename:
            center = centers['AF']
        elif "CF" in filename:
            center = centers['CF']
        ...
        distance = calculate_distance(np.array(vector0), np.array(center))
...
def analyze_data(file_path, save_stat_path, save_plot_path):
    ...
    # extract the normalized score and distance
    distance = data_normalized[:,0]
    # score = data_normalized[:,1]
    
    # # calculate the Pearson correlation coefficient and p-value of the normalized data
    correlation_normalized, p_value_normalized = pearsonr(distance, score)
    # # calculate the Spearman correlation coefficient and p-value of the normalized data
    # correlation_normalized, p_value_normalized = spearmanr(distance, score)

    data_draw = pd.DataFrame({'distance': distance, 'score': score})
    # create the scatter plot of the normalized data
...
# # ########################   analysis  ####################################
# save_file_path = '/stat_results/all_dis_score_newest.xlsx'
# save_stat_path = '/stat_results/pearson/without_norm/results_newest.txt'
# save_plot_path = '/stat_results/pearson/without_norm/dis_score_scaltter_newest.png'
# save_plot_base_path = '/stat_results/pearson/without_norm/5%_multitimes/dis_score_scaltter_newest'
# os.makedirs(os.path.dirname(save_file_path), exist_ok=True)
# os.makedirs(os.path.dirname(save_stat_path), exist_ok=True)
# os.makedirs(os.path.dirname(save_plot_path), exist_ok=True)
# os.makedirs(os.path.dirname(save_plot_base_path), exist_ok=True)


# # save_file_path = './coefficient/all_dis_score.xlsx'
# process_file(all_file_path, save_file_path, centers)
# analyze_data(save_file_path, save_stat_path, save_plot_path)
# analyze_data_5times(save_file_path, save_stat_path, save_plot_base_path)




# ########################      beautify demo  #################################
# save_path = "./clusering/beauty/3.5/2"
# save_path = "./flame/beautify_demo/AM58"
...
def deca_generate(args):
    ...
    for i in tqdm(range(len(testdata))):
        ...
        if args.saveObj:
            objpath = os.path.join(savefolder, name, str(k), acc, 'mesh' , name + f'_{k}.obj')
        ...
        if args.saveVis:
            vispath = os.path.join(savefolder, name, str(k),  acc , 'vis', name + f'_{k}_vis.jpg')
        ...
        if args.saveImages:
            for vis_name in ['inputs', 'rendered_images', 'albedo_images', 'shape_images', 'shape_detail_images', 'landmarks2d']:
                ...
                cv2.imwrite(os.path.join(savefolder, name, str(k),  acc , name + '_' + vis_name +'.jpg'), util.tensor2image(visdict[vis_name][0]))
...
if __name__ == '__main__':
    ...
    deca_generate(parser.parse_args())
