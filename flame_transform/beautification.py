        ...
        if args.saveImages:
            for vis_name in ['inputs', 'rendered_images']:
                if vis_name not in visdict:
                    continue
                vis_dir = os.path.join(savefolder, str(args.k), 'vis', vis_name)
                os.makedirs(vis_dir, exist_ok=True)
                out_np = util.tensor2image(visdict[vis_name][0])
                cv2.imwrite(os.path.join(vis_dir, f"{name}.jpg"), out_np)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--texture', default=False, type=lambda x: x.lower() in ['true', '1'], help="use texture beautification or not")
