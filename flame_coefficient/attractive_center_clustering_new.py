...
# train_file_path = '/workspace/AdBRC/xieyuan/Beautytask/3DFACENet(IJCAI)/flame_coefficient/train_coef.xlsx'
# test_file_path = '/workspace/AdBRC/xieyuan/Beautytask/3DFACENet(IJCAI)/flame_coefficient/test_coef.xlsx'
all_file_path = '/workspace/AdBRC/xieyuan/Beautytask/3DFACENet(IJCAI)/flame_coefficient/all_coef.xlsx'
# save_path = '/workspace/AdBRC/xieyuan/Beautytask/3DFACENet(IJCAI)/flame_coefficient/visualizaiton/41_6'
# save_path = '/workspace/AdBRC/xieyuan/Beautytask/3DFACENet(IJCAI)/flame_coefficient/visualizaiton/all_dim'
save_path = '/workspace/AdBRC/xieyuan/Beautytask/3DFACENet(IJCAI)/flame_coefficient/visualizaiton/all_dim/beauty/4.0'
...
def plot_scatter_and_centers(df, title, save_path, name):
    ...
    plot_data = df[['mica_shape_182', 'mica_shape_201', 'category']]

    # KMeans clustering with 2 centers (beauty and normal)
    kmeans = KMeans(n_clusters=2, random_state=0).fit(plot_data[['mica_shape_182', 'mica_shape_201']])
    ...
    sns.scatterplot(x='mica_shape_182', y='mica_shape_185', hue='category', sizes = (20, 100), data=plot_data, palette='bright')
    centers = kmeans.cluster_centers_
    plt.scatter(centers[:, 0], centers[:, 1], s=100, c='black', marker='X')
    plt.title(title)
    plt.xlabel('mica_shape_182')
    plt.ylabel('mica_shape_185')
...
def plot_scatter_and_centers_shape(df, title, save_path, name):
    # 使用所有80维特征进行聚类
    features = [f'mica_shape_{i}' for i in range(1, 301)]
